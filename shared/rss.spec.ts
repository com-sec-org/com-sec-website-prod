...
describe("RSS", () => {
  ...
  it("generates the repository feed without nonexistent or legacy listing entries", () => {
    ...
    expect(xml).toContain("https://com-sec.io/blog/what-247-security-monitoring-actually-looks-like-for-a-small-company");
    ...
    expect((xml.match(/<item>/g) ?? []).length).toBe(81);
...
describe("RSS images", () => {
  ...
  it("adds images to all 81 repository items without changing other RSS content", () => {
    const routes = canonicalBlogPaths(fs.readFileSync("client/main.tsx", "utf8"));
    const xml = generateRss(allArticles, routes, now);
    expect(xml.match(/<media:content url="https:\/\//g)).toHaveLength(81);
    expect(xml.match(/<media:thumbnail url="https:\/\//g)).toHaveLength(81);
